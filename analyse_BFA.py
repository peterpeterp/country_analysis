...
# members_tas=COU.classify_ensemble(['pr'])
# COU.ensemble_statistic(members_tas,'pr_ensmedian')
# members_tas=COU.classify_ensemble(['spi3m'])
# COU.ensemble_statistic(members_tas,'spi3m_ensmedian')
# COU.save_data()
...
all_spi=COU.classify_ensemble(['spi3m'])
...
COU.model_agreement(ensmedian_spi,members_spi,'diff_droughtRisk_2040s','spi3m_ensmedian_agree')

# plot a map for drought risk
ret=COU.plot_map(COU._periods[grid]['diff_droughtRisk_2040s'][ensmedian_spi],
			grey_area=COU._periods[grid]['diff_droughtRisk_2040s']['spi3m_ensmedian_agree'],
			out_file=COU._working_directory+'/plots/spi_diff_droughtRisk_2040s_ensmedian.png',
			add_mask=plot_mask,
			color_palette=plt.cm.RdBu_r,
			color_range=[-2,2],
			color_label='change in precipitation \n deficit risk [%]',
			title='2040s vs 1986-2005')
...
COU.model_agreement(ensmedian_spi,members_spi,'diff_wetRisk_2040s','spi3m_ensmedian_agree')

# plot map again
ret=COU.plot_map(COU._periods[grid]['diff_wetRisk_2040s'][ensmedian_spi],
			grey_area=COU._periods[grid]['diff_wetRisk_2040s']['spi3m_ensmedian_agree'],
			out_file=COU._working_directory+'/plots/spi_diff_wetRisk_2040s_ensmedian.png',
			add_mask=plot_mask,
...
# plt.tight_layout()
plt.savefig(COU._working_directory+'/plots/spi_overwiew_drought_risk.png',)
...
plt.tight_layout()
plt.savefig(COU._working_directory+'/plots/spi_risk-1.5_diff_2040s_overview.png',)
...
COU.model_agreement(ensmedian_tas,members_tas,'diff_ref_2040s','tas_ensmedian_agree')

ret=COU.plot_map(COU._periods[grid]['diff_ref_2040s']['tas_ensmedian'],
			grey_area=COU._periods[grid]['diff_ref_2040s']['tas_ensmedian_agree'],
			out_file=COU._working_directory+'/plots/tas_diff_2040s_ensmedian.png',
			add_mask=plot_mask,
			color_palette=plt.cm.Reds,
...
###############
members_pr=COU.classify_ensemble(['pr'])
members_pr=members_pr[:-1]
ensmedian_pr=members_pr[-1]
COU.model_agreement(ensmedian_pr,members_pr,'diff_ref_2040s','pr_ensmedian_agree')

ret=COU.plot_map(COU._periods[grid]['rel_diff_ref_2040s']['pr_ensmedian'],
			grey_area=COU._periods[grid]['diff_ref_2040s']['pr_ensmedian_agree'],
			out_file=COU._working_directory+'/plots/pr_diff_2040s_ensmedian.png',
			add_mask=plot_mask,
			color_palette=plt.cm.RdBu,
			color_range=[-10,10],
			color_label='difference in mean \ntemperature [$^\circ C$]',
...
plt.tight_layout()
plt.savefig(COU._working_directory+'/plots/pr_diff_2040s_overview.png',)

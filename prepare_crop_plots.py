...
for crop,crop_short in zip(['rice','wheat','soybean','maize'],['ric','whe','soy','mai']):
    ...
    for iso in all_isos:
        ...
        if os.path.isfile('maps/'+'_'.join([iso,crop,'total','hist'])+'.png')==False:
            ...
            try:
                ...
                print('No shapefile for this country???')
                loaded=False
                ...
                if np.isfinite(np.nanmean(COU._masks['360x720_lat_89.75_-89.75_lon_-179.75_179.75']['lat_weighted'][iso]*mask)):
                    ...
                    for co2 in ['co2']:
                        ...
                        for wlvl,wlvl_name in zip(['1p0','1p5','2p0','2p5','3p0'],['+1.0$^\circ$C','+1.5$^\circ$C','+2.0$^\circ$C','+2.5$^\circ$C','+3.0$^\circ$C']):
                            ax,im,range,x,y,cbar=ens_mean.display_map(period='diff_relative_'+wlvl+'-hist',
                            ...
                            agree=ens_mean.agreement['mean']['year']['diff_relative_'+wlvl+'-hist'].copy()*mask[ens_mean.lat,ens_mean.lon].values
                            try:
                                ax.contourf(ens_mean.lon,ens_mean.lat, agree, levels=[0.5,1.5],colors='none', hatches=['/////'])
                            except:
                                pass
                            smallChange=ens_mean.smallChange['mean']['year']['diff_relative_'+wlvl+'-hist'].copy()*mask[ens_mean.lat,ens_mean.lon].values
                            try:
                                ax.contourf(ens_mean.lon,ens_mean.lat, smallChange, levels=[0.5,1.5],colors='none', hatches=['.....'])
                            except:
                                pass
                            plt.tight_layout(); plt.savefig('maps/'+'_'.join([iso,crop,'total',wlvl])+'_allinfo.png')
                            ...
                            ax,im,range,x,y,cbar=ens_mean.display_map(period='diff_relative_'+wlvl+'-hist',
                                                    ...
                                                    add_mask=mask[ens_mean.lat,ens_mean.lon].values,
                                                    color_range=[-25,25])
                            smallChange[smallChange==0]=np.nan
                            try:
                            ...
                            for j, lab in enumerate(['<-15','-5;-15','5;-5','15;5','>15']):
                                cbar.ax.text(.5, (2 * j + 1) / 10.0, lab, ha='center', va='center',rotation=270,fontsize=8)
                            cbar.ax.get_yaxis().labelpad = 15
                            cbar.ax.set_aspect(10)
                            cbar.ax.set_ylabel('relative change in yield [%]', rotation=270)

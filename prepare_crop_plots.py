...
all_isos=['MOZ','DZA','AGO',  'EGY', 'GNQ', 'BEN', 'NGA', 'NER', 'ZWE', 'NAM', 'GNB', 'SWZ', 'GHA', 'COG', 'SLE', 'ETH', 'COM', 'ERI', 'CPV', 'LBR',\
...
for crop,crop_short in zip(['rice','wheat','soybean','maize'],['ric','whe','soy','mai']):
    ...
    for iso in all_isos:
        ...
        if os.path.isfile('maps/'+'_'.join([iso,'firr','2p0'])+'.png')==False or True:
            ...
            if np.isfinite(np.nanmean(COU._masks['360x720_lat_89.75_-89.75_lon_-179.75_179.75']['lat_weighted'][iso]*mask))==False:
                break
            ...
            for wlvl in ['1p0','1p5','2p0','2p5','3p0']:
                COU.period_model_agreement(ref_name=wlvl+'_noirr',ens_statistic='median',proj_period_names=[wlvl+'_firr'])

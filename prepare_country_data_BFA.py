...
for rcp in ['rcp2p6','rcp4p5','rcp8p5']:
	...
	#SPI
	for months in [3]:
		all_files=glob.glob('/p/projects/tumble/carls/shared_folder/CMIP5_monthly/SPI/SPI_*'+rcp+'*'+str(months)+'m*')
		for in_file in all_files:
			model=in_file.split('/')[-1].split('_')[1]
			COU.country_zoom(in_file,name='spi3m_'+model+'_'+rcp+'_CMIP5_BC',var_name='spi',overwrite=overwrite)




COU.save_data(country_iso+'.nc4')
